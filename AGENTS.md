...
5. **Patch→Site→Phase 邻接阈值**：候选 30 / 50 / 100 / 200 / 300 m 均已产出（P02/P03，2026-09-09）。
   **临时基线 100 m**（S07 人工样本前的工作默认，筛选规则见
   `PVCropCarbon/docs/experiment_design/INDEX.md`）；主敏感性 50 / 200 m；30 / 300 m 仅作极端边界附录。
   **正式主阈值仍待 S07 冻结**（分层高分影像样本上算法同场址标签与人工标签的一致性最大者）。
   `phase_id` 恒由完整 `Site + 建设年份` 生成（如 `S100_000042_2016`）；先构造完整 Phase /
   完整环带，再与县界相交。Site = 边界间距 ≤ 阈值的连通分量，不加直径/面积上限（P02 诊断：
   30–200 m 无 >50 km² 巨型 Site）。
