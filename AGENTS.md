...
| 计算服务器 | `ssh -p 30383 server@8.130.68.96` | 全部大体量栅格/矢量数据与实际计算 |
...
2. **分析坐标系 / 分析网格 / 分辨率**：**待冻结 (E00)**。CLCD/CCD 名义 30 m，但像元原点、
   范围、年度对齐须审计后冻结；面积统计须在等面积坐标系或逐像元真实面积下完成；类别栅格
   重采样只用最近邻。
...
- 主 PV 图斑有 **3 条建设日期异常**，在派生层修正，不改原始 Shapefile。
