...
2. **分析坐标系 / 分析网格 / 分辨率**：**E00/P01 冻结**。拟采用 CLCD 原生网格 ——
   ...
   (X=-2629624.783, Y=5924251.560)。CCD（EPSG:4326）与 PV 图斑（WGS84）重投影到此网格；
   类别栅格重采样只用最近邻；面积/距离统计在此等面积系下完成。像元原点、范围、年度对齐
   在 P01 逐项核对后正式冻结。
...
- 主 PV 图斑（陈月红，30,023 面，WGS84 地理，字段 `OBJECTID / PV_Area(km²) / Inst_time(YYYYMMDD) /
  Inst_year(2010–2022) / Major_type / Lat / Lon`）有 **3 条建设日期异常**，在派生层修正，不改原始 Shapefile：
  `OBJECTID=21400`（`Inst_time=201650831`，月份非法）、`OBJECTID=20046`（`Inst_time=20170721` 但 `Inst_year=2018`）、
  `OBJECTID=20048`（`Inst_time=20190321` 但 `Inst_year=2018`）。另有 95 面 `Major_type=Ocean area`（海上光伏，可能无所属县）。
